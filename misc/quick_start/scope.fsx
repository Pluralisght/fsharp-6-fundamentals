...
let afunction = 
    let a = 10
    let subfunction = a * a
    let a = 5 // this a is a shadow
    subfunction + a

let square x = x * x
let square x : float = x*x // forced to float because of return type
let square (x : float ) = x*x // forced to float because of parameter type
...
let afunction = 
    ...
    let a = 5 // this is a different a, subfunction cloed the previous a
    printfn "%A" (subfunction 2) // 10 * 2
afunction
